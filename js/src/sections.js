...
(function ($) {
    $(document).ready(function () {
        ...
        $('.news-filter-slider').each(function () {
            var _this = this;
            var filters = [];
            ...
            $(this).find('.big-card').each(function () {
                var occ = $(this).attr('data-category');
            ...
            if (filters.size) {
                ...
                $(this).find('.filters').append('<p class="color-grey p2">Filtruj wg kategorii</p>');
                $(this).find('.filters').append('<button class="btn btn-tag" data-ppl-filter="all">Wszystkie</button>');
            }
            // budowanie filtrow
            filters.forEach(function (item, i) {
                var itemLabel = item;
                // itemLabel = itemLabel[0].toUpperCase() + itemLabel.slice(1);
                itemLabel = itemLabel.replaceAll(' ', '-');
                $(_this).find('.filters').append('<button class="btn btn-tag" data-ppl-filter="' + item + '">' + item + '</button>');
            });

            // aktyowanie filtrów tylko z kategori z któego jest artykuł start
        ...
        var allBodyClass = jQuery('body').attr("class"); // wyciagniecie wszystkich klass z body
        var idOfCid = allBodyClass.slice(allBodyClass.indexOf("cid") + 3); // id z body dla danej kategorii 
        var nameofCategory = jQuery('[data-catid="' + idOfCid + '"]').attr("data-category"); // nazwa kategorii
        jQuery('[data-ppl-filter="' + nameofCategory + '"]').click(); // kliknięcie na kategorie 
        // aktyowanie filtrów tylko z kategori z któego jest artykuł  koniec 


        //slider z aktualnościami end 
        // slider z ludzmi
        ...
        $('.section-one-column-slider:not(.video-slider)').each(function () {
            var slider = $(this).find('.slider-wrapper .slider-3-rows');
        ...
        jQuery('.roll-element').hide()
        jQuery('.toggle-element').click(function () {
            if (jQuery('.roll-element').css('display') === 'none') {
                jQuery('.roll-element').show("slow")
                jQuery('.toggle-element a').text('Hide')
            } else {
                jQuery('.roll-element').hide("slow")
                jQuery('.toggle-element a').text('Read more')
            }
        })
        ...
        $('.ppl-slider').each(function () {
            var _this = this;
            var filters = [];
            var dyscyplineFilter = [];
            var slider = $(this).find('.card-wrapper');
            // var listOfTranslatedPositions = [];
            ...
            $(this).find('.biogramAdv').each(function () {
                ...
                var occDyscypline = $(this).attr('data-dyscypline');
                if (occ) filters.push(occ);
                if (occDyscypline) dyscyplineFilter.push(occDyscypline.split(";"));
            });
            filters = new Set(filters.sort()); // usuniecie unikatow

            dyscyplineFilter = new Set(dyscyplineFilter.sort().flat()); // usuniecie unikatow
            dyscyplineFilter.delete("")
            dyscyplineFilter.delete(" ")

            if (filters.size) {
                $(this).find('.slider-controls').before('<div class="filters"></div>');
                $(this).find('.filters').append('<p class="color-grey p2">Filtrowanie po stanowiskach</p>');
            ...
            filters.forEach(function (item, i) {
                ...
                if (document.documentElement.lang == 'en-gb') {
                    switch (itemLabel) {
                        case 'Adiunkt':
                            itemLabel = 'Assistant Professor'
                            break;
                        case 'Asystent':
                            itemLabel = 'Teaching Assistant'
                            break;
                        case 'Profesor':
                            itemLabel = 'Professor'
                            break;
                        case 'Profesor uczelni':
                            itemLabel = 'Associate Professor'
                            break;
                        case 'Wykladowca':
                            itemLabel = 'Lecturer'
                            break;
            ...
            if (dyscyplineFilter.size) {
                $(this).find('.filters').append('<p class="color-grey p2">Filtrowanie po dyscyplinach</p>');
                $(this).find('.filters').append('<button class="btn btn-tag filter-active" data-ppl-filter="all">Wszystkie</button>');
            }

            dyscyplineFilter.forEach(function (item, i) {
                var itemLabel = item;
                itemLabel = itemLabel[0].toUpperCase() + itemLabel.slice(1);
                itemLabel = itemLabel.replaceAll('-', ' ');
                $(_this).find('.filters').append('<button class="btn btn-tag" data-ppl-filter="' + item + '">' + itemLabel + '</button>');
            ...
            slider.slickFilterable({
                ...
                filter: function (category, slider, settings) {
                    ...
                    var dyscypline = $(this).attr('data-dyscypline');

                    if (occupation == category) {
                        return true;
                    }

                    if (dyscypline && dyscypline.includes(category)) {
                        return true;
                    }
        ...
        $('.podyplomowe .secondary-nav .m-l-auto a.btn').fadeOut(0); // wywalenie przycisku na podyplomowych


        if ($('.section-testimony').length > 0) {
            // Policz liczbę elementów testimony-card
            var testimonyCardCount = $('.testimony-card').length;
            if (testimonyCardCount > 3) {
                jQuery('.testimony-slider').each(function () {
            ...
            // Jeśli liczba elementów jest 3 lub mniej, ustaw display: flex na testimony-wrapper
            $('.news-wrapper').css('display', 'flex');
            $('.news-wrapper').css('flex-wrap', 'warap');
            $('.news-wrapper').css('gap', '24px');
        }
    })
})(jQuery)