...
(function($) {
        ...
        setTimeout(function() {
                $('.small-news-slider').animate({ opacity: 1 });
            }, 1000)
            // setTimeout(function() {
            //     $('.icon-arrow-down').parent().attr('style', 'color:black !important')
            // }, 1000)

        //
        // belka z webinarami, zmiana styli w zależności od strony 
        setTimeout(function() {
            var currentURL = window.location.href;
            if (currentURL == "https://swps.pl/") {
                jQuery(".moduletable.announcement").css("position", "inherit")
            } else {
                jQuery(".moduletable.announcement").css("position", "fixed")
            }
            jQuery(".big-card.studies-card").each(function() {
                var link = jQuery(this).find("a").attr("href");
                jQuery(this).attr("onclick", 'window.location.href="' + link + '"')
            })

            jQuery("#aktualnosci .news-item").each(function() {
                var link = jQuery(this).find("a").attr("href");
                jQuery(this).attr("onclick", 'window.location.href="' + link + '"')
            })
        }, 1500)
